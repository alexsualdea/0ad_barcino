...
function placeTeamWall(teams, playerPosition)
{
	print("Placing team walls");
	for (let i = 0; i < teams.length; ++i)
	{
		...
		{
			let playerid = team[j] -1;
			print("playerid=" + playerid);
			center.x += playerPosition[playerid].x;
			center.y += playerPosition[playerid].y;
		}
		center.x = center.x / team.length;
		center.y = center.y / team.length;
		//print("TEAM " + i + " center=" + center.x + "," + center.y);
		var radius = defaultPlayerBaseRadius() * Math.PI;
		let cornerWallElement = 'tower';
		let style = "ptol_stone";

		placeIrregularPolygonalWall(center, radius, cornerWallElement, style, team[0], 0, 8, 0.5, false);
	}
...
if (!isNomad())
{
	...
	{
		print("Found 2 teams, using stronghold player placement");
		const [playerIDs, playerPosition] = createBasesByPattern(
			"stronghold", 
			fractionToTiles(randFloat(0.3, 0.3)), 
			fractionToTiles(randFloat(0.05, 0.1)), 
			15
			);
			
		markPlayerAvoidanceArea(playerPosition, defaultPlayerBaseRadius());
		placeTeamWall(teamsArray, playerPosition);
	}
	else
	{
		print("Teams != 2, random player placement, no walls");
