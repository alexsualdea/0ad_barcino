Trigger.prototype.InitRegicideGame = function(msg)
{
	...
	for (let templateName of cmpTemplateManager.FindAllTemplates(false))
	{
		...
		// AQUI avoid elephant hero units
		if (classes.indexOf("Hero") == -1 || classes.indexOf("Elephant") != -1 ||
		    playersCivs.every(civ => civ != identity.Civ))
			continue;

		regicideGarrison =  true; // AQUI make hero always garrisonable
...
Trigger.prototype.SpawnRegicideHero = function(playerID, heroTemplates, spawnPoints)
{
	for (let heroTemplate of shuffleArray(heroTemplates))
		for (let spawnPoint of spawnPoints)
		{
			...
			hero = hero[0];
			// AQUI
			// print("regicide hero=" + hero);
			// let identityHero = Engine.QueryInterface(spawnPoint, IID_Identity);
			// if (identityHero)
			// {
			// 	//identityHero.ProductionQueue = new ProductionQueue();
			// 	//print("identityHero.ProductionQueue=" + identityHero.ProductionQueue.Schema);
			// 	// identityHero.ProductionQueue.Entities.push("units/template_unit_infantry_melee");
			// }
			// AQUI
