...
PETRA.AttackPlan = function(gameState, Config, uniqueID, type, data)
{
	...
	else if (type == "HugeAttack")
	{
		...
		this.unitStat.ChampMeleeFastMoving = { "priority": 1, "minSize": 3, "targetSize": 15, "batchSize": 3, "classes": ["FastMoving", "Melee", "Champion"],
			"interests": [["strength", 2]] };
		// AQUI avoid attacking with hero in regicide mode
...
PETRA.AttackPlan.prototype.init = function(gameState)
{
	...
	for (let cat in this.unitStat)
	{
		...
		{
			print("this.buildOrders.push -> Unit.classes=" + Unit.classes + " this.unit[cat]=" + this.unit[cat] + " cat=" + cat);
...
PETRA.AttackPlan.prototype.addBuildOrder = function(gameState, name, unitStats, resetQueue)
{
	if (!this.isStarted())
	{
		...
		Debug.print("this.buildOrders.push (2) -> Unit.classes=" + Unit.classes + " this.unit[name]=" + this.unit[name] + " name=" + name);
...
PETRA.AttackPlan.prototype.assignUnits = function(gameState)
{
	...
	if (!this.canBuildUnits)
	{
		for (let ent of gameState.getOwnUnits().values())
		{
			if (ent.getMetadata(PlayerID, "allied") || !this.isAvailableUnit(gameState, ent))
				continue;
			if (gameState.getVictoryConditions().has("regicide") && ent.hasClass("Hero"))
			{
				continue;
			}
			Debug.print("Assign to plan: " + ent);
	...
	if (this.type == "Raid")
	{
		...
		for (let ent of gameState.getOwnUnits().values())
		{
			...
			Debug.print("Assign to plan2: " + ent);
			ent.setMetadata(PlayerID, "plan", plan);
			this.unitCollection.updateEnt(ent);
			added = true;
		}
	...
	for (let ent of gameState.getOwnEntitiesByRole(undefined, true).values())
	{
		...
		Debug.print("Assign to plan3: " + ent);
		ent.setMetadata(PlayerID, "plan", plan);
		this.unitCollection.updateEnt(ent);
		added = true;
	}
	...
	for (let ent of gameState.ai.HQ.attackManager.outOfPlan.values())
	{
		...
		Debug.print("Assign to plan4: " + ent);
		ent.setMetadata(PlayerID, "plan", plan);
		this.unitCollection.updateEnt(ent);
		added = true;
	}
	...
	for (let ent of gameState.getOwnEntitiesByRole("worker", true).values())
	{
		...
		Debug.print("Assign to plan5: " + ent);
		ent.setMetadata(PlayerID, "plan", plan);
		this.unitCollection.updateEnt(ent);
		added = true;
	}
...
PETRA.AttackPlan.prototype.reassignFastUnit = function(gameState)
{
	for (let ent of this.unitCollection.values())
	{
		...
		print("Assign to raid '" + raid.name + "': " + ent);
 ...
 * 
 * @param {*} ent 
 * @param {*} targetClassesSiege 
 * @param {*} veto 
 */
PETRA.AttackPlan.prototype.siegeAttackAnything = function(ent,  targetClassesSiege, veto)
{
	if (!ent.hasClass("Ranged"))
	{
		let targetClasses;
		if (this.target.hasClass("Gate"))
		{
			targetClasses = { "attack": targetClassesSiege.attack, "avoid": targetClassesSiege.avoid.concat("Ship"), "vetoEntities": veto };
		}
		else
		{
			let av = targetClassesSiege.avoid.concat("Ship");
			targetClasses = { "attack": targetClassesSiege.attack, "avoid": av, "vetoEntities": veto };
		}
		Debug.print("siege attackMove: " + this.target);
		ent.attackMove(this.targetPos[0], this.targetPos[1], targetClasses);
	}
	else
	{
		Debug.print("siege attackMove(2): " + this.target);
...
PETRA.AttackPlan.prototype.update = function(gameState, events)
{
	...
	if (this.state == "")
	{
		...
		Debug.print("this.target._templateName=" + this.target._templateName);
		Debug.print("this.target._classes=" + this.target._templateName._classes);
		...
		else
		{
			...
			{
				Debug.print("targetClassesUnit: 1");
		...
		{
			if (this.target.hasClass("Gate") || this.target.hasClass("CivilCentre")) // AQUI when attacing gate or civil centre avoid walls
			{
				Debug.print("targetClassesSiege: 1");
				targetClassesSiege = { "attack": ["Structure"], "avoid": ["Wall"], "vetoEntities": veto }; 
			}
			else
			{
				Debug.print("targetClassesSiege: 2");
				targetClassesSiege = { "attack": ["Structure"], "avoid": [], "vetoEntities": veto }; 
			}
		}
		else
		{
			Debug.print("targetClassesSiege: 3");
		...
		let maxlen = 15
		//let lgth = this.unitCollUpdateArray.length < maxlen || this.startingAttack ? this.unitCollUpdateArray.length : 10; 
		let lgth = this.unitCollUpdateArray.length; // AQUI lgth = this.unitCollUpdateArray.length < maxlen || this.startingAttack ? this.unitCollUpdateArray.length : 10; 

		// count siege units
		let siegeUnitsCount = 0;
		for (let i = 0; i < lgth; i++)
		{
			let ent = gameState.getEntityById(this.unitCollUpdateArray[i]);
			let siegeUnit = PETRA.isSiegeUnit(ent);
			if (siegeUnit)
				siegeUnitsCount++;
		}

		Debug.print("--> this.unitCollUpdateArray.length=" + this.unitCollUpdateArray.length);
		for (let check = 0; check < lgth; check++)
		{
			...
			if (siegeUnit)
			{
				...
				if (mStruct.length)
				{
					...
					if (this.target._templateName.search("civil_centre") >= 0) // AQUI
					{
						for (let i in mStruct)
						{
							if (mStruct[i].hasClass("Wall") || mStruct[i].hasClass("Wall Tower")) // remove walls
								mStruct.splice(i, 1)
						}
						if (mStruct.length > 0)
						{
							let r = randIntExclusive(0, siegeUnitsCount);
							if (r <= siegeUnitsCount * (1/siegeUnitsCount)) // assign one siege unit directly
							{
								Debug.print("siege attack: civil centre.");
								ent.attack(this.target.id(), PETRA.allowCapture(gameState, ent, this.target)); 
							}
							else
							{
								// remove civil centre from the eligibles
								for (let m in mStruct)
								{
									if (this.target.id() == mStruct[m].id())
										mStruct.splice(m, 1);
								}
								if (mStruct.length > 0)
								{
									let rand = randIntExclusive(0, mStruct.length * 0.2);
									Debug.print("siege attack: " + mStruct[rand]);
									ent.attack(mStruct[rand].id(), PETRA.allowCapture(gameState, ent, mStruct[rand]));
								}
								else
								{
									this.siegeAttackAnything(ent, targetClassesSiege, veto);
								}
							}
						}
						else
						{
							Debug.print("siege attack: civil centre. (2)");
							ent.attack(this.target.id(), PETRA.allowCapture(gameState, ent, this.target)); 
						}
					}
					else
					{
						...
						{
							Debug.print("siege attack: (gate) mStruct.length >0 -> mStuct[0]=" + mStruct[0]);
							ent.attack(mStruct[0].id(), PETRA.allowCapture(gameState, ent, mStruct[0]));
						}

						else
						{
							Debug.print("siege attack: (else) mStruct.length >0 -> mStuct[0]=" + mStruct[0]);
							let rand = randIntExclusive(0, mStruct.length * 0.2);
							ent.attack(mStruct[rand].id(), PETRA.allowCapture(gameState, ent, mStruct[rand]));
						}
					}
			...
			else
			{
				...
				if (mUnit.length)
				{
					...
					if (this.target._templateName.search("civil_centre") >= 0) // AQUI
					{
						let r = randIntExclusive(0, 10);
						if (r > 5 ) // assing 50% of soldiers
						{
							Debug.print("unit attack/capture: civil centre");
							ent.attack(this.target.id(), PETRA.allowCapture(gameState, ent, this.target)); 
						}
						else
						{
							...
							for (let i in mUnit)
							{
								if (mUnit[i].hasClass("Wall") || mUnit[i].hasClass("Wall Tower")) 
									mUnit.splice(i, 1)
							}
							
							if (mUnit.length > 0)
							{
								Debug.print("unit attack/capture: " + mUnit[rand]);
								ent.attack(mUnit[rand].id(), PETRA.allowCapture(gameState, ent, mUnit[rand]));
							}
							else
							{
								Debug.print("unit attack/capture: civil centre (2)");
								ent.attack(this.target.id(), PETRA.allowCapture(gameState, ent, this.target)); 
							}
						}
					}
					...
					{
						Debug.print("unit attack/capture(2): " + mUnit[rand]);
						ent.attack(mUnit[rand].id(), PETRA.allowCapture(gameState, ent, mUnit[rand]));
					}
				}
				...
				{
					Debug.print("unit attack/capture: this.isBlocked -> ent.attack("  + this.target.id() + ", false)");
					ent.attack(this.target.id(), false);
				}
				else if (API3.SquareVectorDistance(this.targetPos, ent.position()) > 10000) // AQUI 2500
				{
					...
					Debug.print("unit attackMove (3): API3.SquareVectorDistance(this.targetPos, ent.position()) > 2500 -> this.targetPos=" + this.targetPos);
					ent.attackMove(this.targetPos[0], this.targetPos[1], targetClasses);
				}
				else
				{
					...
					if (mStruct.length)
					{
						...
						if (mStruct[0].hasClass("Gate"))
						{
							Debug.print("unit attack/capture(3): " + mStruct[0]);
							ent.attack(mStruct[0].id(), false);
						}
						else
						{
							let rand = randIntExclusive(0, mStruct.length * 0.2);
							Debug.print("unit attack/capture(4): " + mStruct[rand]);
							ent.attack(mStruct[rand].id(), PETRA.allowCapture(gameState, ent, mStruct[rand]));
						}
					}
