...
var ping_entities = undefined;

// flare flag 
var flag_template = undefined;

function loadFlagTEmplate()
{
	let cmpTemplateManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_TemplateManager);
	for (let templateName of cmpTemplateManager.FindAllTemplates(true))
	{
		if (!templateName.startsWith("structures/flag"))
		{
			continue;
		}
		else
		{
			flag_template = templateName;
			break;
		}
	}
}
...
var g_Commands = {
	...
	"flare": function(player, cmd, data)
	{
		if (ping_entities != undefined)
			return;

		if (flag_template == undefined)
			loadFlagTEmplate();

		let template = flag_template;
		ping_entities = [];
		for (var pl in cmd.players)
		{
			...
			if (cmpEntOwnership)
			{
				cmpEntOwnership.SetOwner(cmd.players[pl]);
			}
			cmpEntPosition.JumpTo(cmd.x, cmd.z);
			Engine.PostMessage(ent, MT_MinimapPing);
			PlaySound("attacked_gaia", ent);
			ping_entities.push(ent);
		}
	},

	"unflare": function(player, cmd, data)
	{
		if (ping_entities != undefined)
		{
			for (var i in ping_entities)
				Engine.DestroyEntity(ping_entities[i]);
		}
		ping_entities = undefined;
	},
